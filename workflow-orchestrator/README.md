...
평가한 기준선(10개 사례 모음, 실제 위임 실행) 이후 규칙 세 가지를 강화했다: 인계·상태 문서를 포함해 checkout 안의 어떤 파일이든 읽는 것은 위임할 프로젝트 조사이고, 작고 독립적인 변경은 최소 인원으로 진행하며 선택적 단계보다 먼저 보고해 세션의 turn·시간 예산을 지키고, 모든 반영 경로는 GATE, VERIFY, 구현하지 않은 작업자의 리뷰를 명시하되 commit/push 승인을 merge·배포로 확장하지 않는다.

## 버전 1.5.0 참고

버전 1.4.0과 1.4.1은 조율자가 요청에 명시된 브리프·상태 파일 하나를 열고 디렉터리를 나열할 수 있도록 조율자 계약을 완화했다. Opus 5로 진행한 쌍 비교 평가에서 이 완화는 순손실로 나타났다: 사용자가 그 읽기를 작업자에게 배정했는데도 조율자가 BRIEF.md를 열었고(TC-006 실패), 엄격한 1.2.0 문구는 같은 조건에서 10개 사례를 모두 통과했다. 1.5.0은 1.3.0 계약을 그대로 복원한다. 평가 기준은 이름만 나열하는 디렉터리 목록과 사용자 요청에 명시된 파일 읽기를 허용하는데, 모델이 스킬이 작동하기 전에 그렇게 하기 때문이다. 그러나 스킬은 계속 조율자에게 그러지 말라고 요구한다.
